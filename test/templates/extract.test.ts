...
describe("Chitking template extraction helpers", () => {
  it("resolves the source template root", () => {
    ...
    expect(existsSync(getChitkingRuntimeTemplatePath("config.yaml"))).toBe(
      true,
    );
    for (const command of EXPECTED_CK_COMMANDS) {
      expect(
        existsSync(getChitkingRuntimeTemplatePath("commands", `${command}.md`)),
      ).toBe(true);
    }
    expect(
      existsSync(
        getChitkingRuntimeTemplatePath("skills", "chitking-workflow.md"),
      ),
    ).toBe(true);
    expect(
      existsSync(
        getOpenCodeTemplatePath("plugins", "inject-chitking-context.js"),
      ),
    ).toBe(true);
    expect(existsSync(getCodexTemplatePath("config.toml"))).toBe(true);
    expect(
      readFileSync(getCodexTemplatePath("config.toml"), "utf-8"),
    ).not.toContain("project_doc_fallback_filenames");
  });
  ...
  it("can resolve copied dist templates after build", () => {
    ...
    if (existsSync("dist")) {
      ...
      expect(
        existsSync(
          path.resolve("dist", "templates", "chitking", "config.yaml"),
        ),
      ).toBe(true);
      expect(
        existsSync(
      ...
      expect(
        existsSync(path.resolve("dist", "templates", "codex", "config.toml")),
      ).toBe(true);
    }
  });
});
