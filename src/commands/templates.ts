...
export function predictRoleContractContent(role: RoleDefinition): string {
  const warnings = role.warnings.map((warning) => `- ${warning}`).join("\n");
  const stopConditions = role.prompt.stop_conditions
    .map((condition) => `- ${condition}`)
    .join("\n");

  return `# Chitking Predict Role\n\n## Objective\n\n${role.prompt.objective}\n\n## Required Inputs\n\n- Theory brief.\n- Current claim and capability gap.\n- Verification obligations.\n- Existing evidence and failed paths.\n- The current per-thread packet from \`chitking dispatch --role predict\`.\n\n## Required Output Shape\n\nEvery prediction must include these labeled fields:\n\n- **Claim**: A concrete statement about the world, mechanism, or behavior under investigation.\n- **Source**: The cited source that grounds the claim (paper, prior experiment, model, observation). No Source = stop.\n- **Predicted Effect**: What you expect to observe or measure if the claim holds.\n- **Falsification Criterion**: The specific observation that would refute the prediction. No Falsification Criterion = stop.\n\n## Hard Boundaries\n\n- Do not create implementation tasks.\n- Do not assign work to build, Executor, or any implementation role.\n- Do not hand Predict output directly to build or Executor; route through plan or review first.\n- Do not present a prediction as an approved next safe action.\n- Predictions are advisory until human, oracle, planner, or reviewer review applies them.\n\n## Warnings\n\n${warnings}\n\n## Stop Conditions\n\n${stopConditions}\n`;
}
  ...
  for (const [roleName, role] of Object.entries(config.roles)) {
    const contractContent =
      roleName === "dreamer"
        ...
        : roleName === "predict"
          ? predictRoleContractContent(role)
